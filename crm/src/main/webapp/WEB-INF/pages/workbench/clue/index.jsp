...
<head>
	...
	$(function(){
		...
		$("#createClueBtn").click(function () {
			//重置表单,只能使用dom对象的reset();
			$("#createClueForm")[0].reset();
			//弹出模态窗口
			$("#createClueModal").modal("show");
		})

		$("#saveCreateClueBtn").click(function(){//点击之后,收集所有参数,发送ajax请求
			...
			//这里加个正则表达式来约束公司网站
			var websiteTest=/^(?=^.{3,255}$)[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+$/;
			if (!websiteTest.test(website)) {
				alert("网站格式不正确");
				return;
			}
			//这里加个正则表达式来约束手机号
			var mphoneTest=/^[1][3,4,5,7,8][0-9]{9}$/;
			if (!mphoneTest.test(mphone)) {
				alert("手机号格式不正确");
				return;
			}*/
